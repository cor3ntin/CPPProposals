...
\ednote{TODO: this should work? https://compiler-explorer.com/z/Mo6bobv4h}
...
\tcode{all_of}, \tcode{any_of}, etc can then be implemented in a way that properly supports subsumption.
...
when performing a lookup. This is also consistent with previous work (for example \paper{N3595} and \paper{P0934R0}),
and the general consensus towards ADL.

\subsection{Equivalence of atomic constraints}

One interesting concept to consider is \tcode{tuple_of}, which would allow constraining a function on a tuple-like of integrals for example - a frequent use case in scientific
computation.
...
Let us consider a \tcode{range_of} concept constraining its value type with a concept template parameter.
...
We can rewrite our \tcode{range_of} concept to take extra argument forwarded to the concept parameter
...
And all hope of subsumption is lost (concepts cannot be class members for very good reason - it would fundamentally break subsumption - so \tcode{apply} is a \tcode{bool} variable - an atomic constraint).
