...
%
For an expression \tcode{E} that is a subexpression of the \grammarterm{constraint-expression} of a concept-id \tcode{C<A$_1$, A$_2$, ..., A$_n$>},
the \defn{substitutable concept parameters} of \tcode{E} is set of concept template parameters named by \tcode{E}
which denotes a template parameter of \tcode{C} and for which there is a corresponding non-dependent argument in \tcode{C<A$_1$, A$_2$, ..., A$_n$>}.
...
\added{For a concept-id \tcode{C<A$_1$, A$_2$, ..., A$_n$>}}
\begin{itemize}
\begin{addedblock}
...
\item When \tcode{C} names a concept template parameter, the normal form of a concept-id \tcode{C<A$_1$, A$_2$, ..., A$_n$>} is a concept-dependent constraint
 whose concept-id is  \tcode{C<A$_1$, A$_2$, ..., A$_n$>}.
...
\begin{removedblock}
 substituting \tcode{A$_1$, A$_2$, ..., A$_n$} for
...
\begin{addedblock}
\item
For a \grammarterm{fold-operator} that is either \tcode{\&\&} or \tcode{||},
\begin{itemize}
\item If \tcode{E} contains one or more unexpanded parameter packs $P_k$ that denotes a substitutable concept parameter,
let $N_k$ be the number of arguments specified for each pack $P_k$.
If the value of each $N_k$ differs, the normal form of \tcode{( E \grammarterm{fold-operator} ... )} is a fold expanded constraint \iref{temp.constr.fold}
whose constraint is the normal form of \tcode{E}.

Otherwise the normal form of \tcode{( E \grammarterm{fold-operator} ... )} is the conjunction (for a \grammarterm{fold-operator} that is \tcode{\&\&}),
or the disjunction (for a \grammarterm{fold-operator} that is \tcode{||}), of the normal form of each $E_i$ for $0 < i < N_0$.
If producing the normal form of a subexpression of $E_i$ requires to subsitute a concept-name refering to a $P_k$, it is substituted by the $i^\text{th}$ corresponding template argument.
...
\item Otherwise (if \tcode{E} does not contain an unexpanded parameter pack that denote a substitutable concept template parameter pack), the normal form of \tcode{( E \grammarterm{fold-operator} ... )} is a fold expanded constraint\iref{temp.constr.fold}
whose constraint is the normal form of \tcode{E} and
whose \grammarterm{fold-operator} is \grammarterm{fold-operator}.
\end{itemize}
\end{addedblock}
\begin{removedblock}
\item
