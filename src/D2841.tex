...
\textbf{During partial ordering, if a \tcode{concept-id} that names a concept template parameter appears in the associated constraints of a declaration \tcode{D}, \tcode{D} is never at least as constrained as another constrained declaration}
...
Ultimately, while whe have a slight preference for option 1, we submit the following question to EWG:
...
\begin{note}
    ...
    \grammarterm{\changed{type}{template-template}-parameter}
...
denotes the type in a non-type \grammarterm{parameter-declaration}.
\begin{wfootnote}
...
\ednote{We should get rid of that}
...
A \grammarterm{template-template-parameter} \tcode{P} whose \grammarterm{identifier} does not follow an ellipsis defines its
...
\begin{addedblock}
A \grammarterm{template-argument} for a \grammarterm{template-template-parameter} shall be an \grammarterm{id-expression} denoting
\begin{itemize}
...
\begin{addedblock}
The associated constraints \tcode{C} of a declaration \tcode{D} are \emph{subsumption-eligible} unless \tcode{D} is a \grammarterm{template-declaration} and a subexpression of \tcode{C} denotes a concept template parameter of D.
\end{addedblock}
...
template arguments \added{names a \grammarterm{template-template-parameter} or }is a dependent type or an expression that is type-dependent
