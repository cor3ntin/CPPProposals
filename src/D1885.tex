...
\item Add some prose clarifying that the intent is that UCS2/UCS4 are valid return values on platforms with little and big endianess. 
\end{itemize}
...
\subsection{CHAR_BIT}
SG-16 had unresolved questions about how notably UTF-16 should be encoded on a platform where CHAR_BIT is 16 bits or more.
Tom Honermann did some research, and there does not seem to exist such scenario, to the best of our knowledge, based on public information.
The author has no use cases for using P1885 on DSPs system.
We would also like to point out that P1885 is designed to be freestasnding in the future, but it is currenly none, and there is seemingly no non-8 bits
non-freestandiung system.
As such, the wording mandates \tcode{CHAR_BIT == 8}, so that the wording doesn't have to account for hypotheticals.
If the need to relax this limitation emerges, it should be driven by people with expert knowledge of such environment.
...
\begin{itemize}
\item \tcode{text_encoding} is described as holding an encooding \emph{scheme},
\item \tcode{text_encoding::literal} and \tcode{text_encoding::wide_literal} describe the encoding scheme of the object representation.
\item In particular, an encoding scheme is a sequence of octets (independently of\tcode{CHAR_BIT}), the general idea being that reinterpreting an encoded string as a \tcode{char*}, along with the encoding name should be decodable by \tcode{iconv}, assuming the encoding name is known of \tcode{iconv}.
\item Because ISO 10646 definition of UTF16 is not consistent with that of Unicode, and because SG16 prefers UTF-16 to be returned over UTF-16LE, for example, we introduce a wording mechanism to substitute UTF16-LE by UTF16. The goal is to provide a consistent, useful result to users while conforming to ISO.
...
Let \tcode{E} be a \tcode{text_encoding} object representing the encoding scheme associated with the object representation of ordinary string literals [lex.charset].
...
Let \tcode{E} be a \tcode{text_encoding} object representing the implementation-defined character encoding scheme of the environment.
...
Let \tcode{E} be a \tcode{text_encoding} object representing the implementation-defined encoding scheme associated with the locale \tcode{*this}.
