...
no name is present when a text_encoding object is constructed by \tcode{text_encoding(text_encoding::other)}.
...
\item \tcode{COMP_NAME} becomes an exposition only static member function  \tcode{comp-name}
\item Reword \tcode{comp-name} to avoid an englisgh grammar ambiguity.
\item Fix feature macro placeholder value
\item \tcode{enum class id} => add a \seebelow
\item s/constexpr friend/friend constexpr
\item Improve \tcode{aliases()} avd \tcode{literal()} descriptions
\end{itemize}

Following LWG small group review (12th June 2022)
\begin{itemize}
    \item Add a synopsis for the \tcode{text_encoding} header.
    \item Move the definition of \tcode{text_encoding::id} in its own subsection.
    \item Add Overview, General, and Members headings.
    \item Correctly use square brackets to denote an example.
    \item Remove nonsensical \tcode{text_encoding::} qualification from the declaration of \tcode{environment_is}.
    \item Reorder the bullets in \tcode{text_encoding::id}.
    \item Fix the \tcode{text_encoding} invariant description (\tcode{==other} => \tcode{!= unknown} in the second bullet).
    \item Add a comma in \placeholder{COMP_NAME} (equal, ignoring...).
    \item all elements which => all elements that
    \item \tcode{text_encoding(string_view)} constructor: basic source character set => basic character set, add reference to lex.charset.
    \item Turn that constructor's bullets into sentences.
    \item Add missing parentheses in \tcode{ranges::contains(aliases, ...)}.
    \item In \tcode{name()} NTMBs => NTBS (unlike what i said in the meeting, this can never be a multi byte string because the constructor has a precondition, which needs to be there for COMP_NAME to work with exotic shift state encodings).
    \item In \tcode{name()}, use a different notation for the range.
    \item Specify that \tcode{text_encoding} is a trivially copyable type.
    \item In the description of aliases_view
    \begin{itemize}
        \item \tcode{alias_view} models and satisfies => \tcode{alias_view} models.
        \item \tcode{model same_as<const char*>} => denote \tcode{const char*}.
    \end{itemize}
\end{itemize}
...
\begin{codeblock}
#define __cpp_lib_text_encoding 2023XX (**placeholder**) // also in \tcode{text_encoding}
\end{codeblock}


\begin{quote}

\rSec1[localization]{Localization}
...
\begin{libsumtab}{Localization library summary}{localization.summary}
    \char`[locales] & Locales                   &   \tcode{<locale>}    \\
    \char`[locale.categories] & Standard \tcode{locale} & categories    \\
    \char`[c.locales] & C library locales       &   \tcode{<clocale>}   \\
    \added{\char`[text.encoding]} & \added{Text encodings identification}  &   \added{\tcode{<text_encoding>}}   \\
\end{libsumtab}

\ednote{The intent is to [text.encoding] to a better section at a letter date, as described in \href{https://github.com/cplusplus/draft/issues/5226}{GH\#5226} }.

\begin{addedblock}
\rSec2[text.encoding.syn]{Header \tcode{<text\_encoding>} synopsis}

\ednote{The position of this subclause in [localization] is left as the editors discretion}.
...
\rSec2[text.encoding]{class \tcode{text_encoding}}
...
struct text_encoding {
    ...
    private:
        ...
        static constexpr bool @\placeholder{comp-name}@(string_view a, string_view b); // \expos
    };
}
\end{codeblock}
\end{addedblock}
...
\tcode{text_encoding} is a trivially copyable type ([basic.types.general]).
...
Each known registered character encoding is identified by an enumerator in \tcode{text_encoding::id}, and has a set of zero or more \defn{alias}{es}.
...
No two aliases or primary names of distinct registered character encodings are equivalent when compared by \tcode{text_encoding::\placeholder{comp-name}}.
...
An object \tcode{e} of type \tcode{text_encoding} such that \tcode{e.mib() == text_encoding::id::unknown} is \tcode{false} and \tcode{e.mib() == text_encoding::id::other} is \tcode{false} maintains the following invariants:
\begin{itemize}
\item \tcode{e.name() == nullptr} is \tcode{false}.
...
\begin{itemdescr}
    \preconditions
    \begin{itemize}
        \item \tcode{enc} represents a string in the ordinary literal encoding consisting only of elements of the basic character set [lex.charset].
        \item \tcode{enc.size() <= max_name_length} is \tcode{true}.
        \item \tcode{enc.contains('\textbackslash 0')} is \tcode{false}.
    \end{itemize}
    \postconditions
    \begin{itemize}
         \item If there exists a primary name or alias \tcode{a} of a known registered character encoding such that \tcode{\placeholder{comp-name}(a, enc)} is \tcode{true}, \tcode{\exposid{mib_}} has the value of the enumerator of \tcode{id} associated with that registered character encoding. Otherwise, \tcode{\exposid{mib_} == id::other} is \tcode{true}.
         \item  \tcode{enc.compare(\exposid{name_}) == 0} is \tcode{true}.
    \end{itemize}
\end{itemdescr}
...
\begin{itemdescr}
    ...
    \postconditions
    \begin{itemize}
       \item  \tcode{mib_ == i} is \tcode{true}.
       \item  If \tcode{(mib_ == id::unknown || mib_ == id::other)} is \tcode{true}, \tcode{strlen(\exposid{name_}) == 0} is \tcode{true}. Otherwise,
       \tcode{ranges::contains(aliases(), string_view(\exposid{name_}))} is \tcode{true}.
    \end{itemize}
\end{itemdescr}
...
\end{itemdecl}
\begin{itemdescr}
\returns \tcode{\exposid{name_}} if \tcode{(\exposid{name_}[0] != '\textbackslash 0')}, \tcode{nullptr} otherwise;

\remarks
If \tcode{name() == nullptr} is \tcode{false}, \tcode{name()} is an NTBS and accessing elements of\\
\tcode{name\_} outside of the range \tcode{name() + [0, strlen(name()) + 1)} is undefined behavior.
\end{itemdescr}
...
Each element in \tcode{r} is a non-null, non-empty NTBS encoded in the literal character encoding and comprising only characters from the basic character set.
...
\returns A \tcode{text_encoding} object representing the ordinary character literal encoding [lex.charset].
...
\begin{itemdecl}
static constexpr bool @\placeholder{comp-name}@(string_view a, string_view b);
\end{itemdecl}
\begin{itemdescr}

\returns \tcode{true} if the two strings a and b encoded in the ordinary literal encoding are equal, ignoring, from left-to-right,
\begin{itemize}
    \item all elements that are not digits or letters [character.seq.general],
    \item character case, and
    \item any sequence of one or more \tcode{0} characters not immediately preceded by a numeric prefix, where a numeric prefix is a sequence consisting of a digit in the range [1-9] optionally followed by one or more elements which are not digits or letters
\end{itemize}
...
\begin{example}
    \begin{codeblock}
        assert(@\placeholder{comp-name}@("UTF-8", "utf8") == true);
        assert(@\placeholder{comp-name}@("u.t.f-008", "utf8") == true);
        assert(@\placeholder{comp-name}@("ut8", "utf8") == false);
        assert(@\placeholder{comp-name}@("utf-80", "utf8") == false);
    \end{codeblock}
\end{example}
\end{itemdescr}
...
Both \tcode{ranges::range_value_t<text_encoding::aliases_view>} and \tcode{ranges::range_reference_t<text_encoding::aliases_view>} denote \tcode{const char*}.
...
\begin{itemdescr}
    ...
    If \tcode{a.\exposid{mib_} == id::other \&\& b.\exposid{mib_} == id::other} is \tcode{true},\\ then
    \tcode{\placeholder{comp-name}(a.\exposid{name_}, b.\exposid{name_})}.
...
\begin{itemdescr}
    \returns \tcode{encoding.\exposid{mib_} == i}.
...
\begin{note}
    ...
    \begin{itemize}
        ...
        \item \tcode{csIBBM904} is mapped to \tcode{text_encoding::id::IBM904}, and
        \item the "cs" prefix is removed from other names.
    \end{itemize}
\end{note}


\rSec2[text.encoding.hash]{Hash specialization}
