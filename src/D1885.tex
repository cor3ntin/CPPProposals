...
\begin{colorblock}
    ...
    Wide Literal Encoding: x-wide16-EBCDIC-US (iana mib: 2078)
...
\begin{itemize}
    \item Add some prose to explain the expected behavior of the \tcode{wide_environment} methods, especially that it does not aim to return encodings always suitabke for use with the wide character functions. This is accompanied by wording notes.
    ...
    This is for example to allow implementations to return the Big5 encoding, as the name as registered may actually describe slightly different characters sets. But there are existing practices to respect and we do not want to constrain implementation too much.
...
\begin{itemize}
    ...
    Note that this view is not \tcode{common_range} because it can be implemented more efficiently without that requierement and, being copyable it can be adapted into one.
...
The intent of this proposal is that the names refer to the character ecodings as described by IANA.
...
While this would be conforming, it would more helpful if the implementation was impervious to such modification of the environment.
...
On implementations where the implementers also control the libc, better stategies may be available.

It useful to get the locale of the environment as this represents the encoding that command-line arguments, 
environment variables, and non-redirected standard streams are likely to use.

The encoding of the current locale, may be different - because the global locale is initially set to "C", and users can set an arbitrary global locale,
and therefore that global locale-associated encoding may be different from the environment encoding.
...
of aliases for a particular encoding, which is slightly efficient that what is possible so this is not proposed.
But mostly we found little uses for it to be a \tcode{common_range}.
...
tools that need encoding names.
...
As such, to be helpful, the paper intends for implementation to return the encoding they expect,
...
Further work is needed for the standard to reflect the reality of existing, widely deployed implementations.
...
In the general case, the size of code units of individual encodings is not exposed by IANA, but most encoding are sufficiently defined to understand 0-padded forms as not being that encoding.

For example, the specificatio of UTF-8 reads
...
Both these ISO 646 characters encodings have aliases that conflict with other aliases under COMP_NAME and are excluded, as they felt into disuse a long time ago.
This was necessary to allow compatibility with the Unicode-recommended name matching.


\section{Polls}

\subsection{SG-16 - 6 octoober 2021}
...
\pnum A \defn{registered character encoding} is a character encoding form in the IANA Character Sets registry.
\begin{note}
...
How a \tcode{text_encoding} object is determined to be representative of a character encoding implemented in the translation or execution environment is implementation-defined.
...
\begin{itemdescr}
Let \tcode{E} be \returns a \tcode{text_encoding} object representing the encoding scheme of the object representation of ordinary string literals [lex.charset].
...
\begin{itemdescr}
Let \tcode{E} be \returns a \tcode{text_encoding} object representing the encoding scheme of the object representation of wide string literals [lex.charset].
...
\begin{itemdescr}
Let \tcode{E} be a \tcode{text_encoding} object representing the implementation-defined character encoding of the environment.
On a POSIX implementation, this is the encoding associated with the POSIX locale denoted by the empty string \tcode{""}.

\returns \tcode{\emph{SUBSTITUTE_UTF_ENCODING}{(E)}}.

\begin{note}
    This function is not affected by calls to \tcode{setlocale}.
    It is unspecified whether this function is affected by changes to environment variables
    during the lifetime of the program.
\end{note}

\recommended Implementations should return a value that is not affected by calls to the POSIX functions \tcode{setenv} 
...
\begin{itemdescr}
Let \tcode{E} be a \tcode{text_encoding} object representing the implementation-defined wide character encoding of the environment.
On a POSIX implementation, this is the encoding associated with the POSIX locale denoted by the empty string \tcode{""}.

\returns \tcode{\emph{SUBSTITUTE_UTF_ENCODING}{(E)}}.

On a POSIX implementation, this is the wide encoding associated with the POSIX locale denoted by the empty string \tcode{""}.
...
\begin{itemdescr}
  \returns The execution encoding associated with the locale \tcode{*this}.
\end{itemdescr}
...
\begin{itemdescr}
  \returns The wide execution encoding associated with the locale \tcode{*this}.
\end{itemdescr}
\end{addedblock}
...
Many thanks to Victor Zverovich, Thiago Macieira, Jens Maurer, Tom Honermann, Tomasz Kamiński, Hubert Tong, 
and others for reviewing this work and providing valuable feedback.
