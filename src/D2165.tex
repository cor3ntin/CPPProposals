...
=  @\placeholder{tuple-like-impl}@<remove_cvref_t<T>>;
...
Let \tcode{\exposid{FWD}(u)} be \tcode{static_cast<UTuple>(u)}.
...
\begin{itemdescr}
    ...
    \begin{itemize}
        \item
        \tcode{is_assignable_v<T1\&, decltype(std::get<0>(std::forward<P>(p)))>} is \tcode{true}, and
        \item
        \tcode{is_assignable_v<T2\&, decltype(std::get<1>(std::forward<P>(p)))>} is \tcode{true}.
    \end{itemize}

    \effects
    Assigns \tcode{std::get<0>(std::forward<decltype(p)>(p))} to \tcode{first}\\ and
    \tcode{std::get<1>(std::forward<decltype(p)>(p))} to \tcode{second}.
...
\begin{itemdescr}
    ...
    \begin{itemize}
        \item
        \tcode{is_assignable_v<const T1\&, decltype(std::get<0>(std::forward<P>(p)))>} is \tcode{true}, and
        \item
        \tcode{is_assignable_v<const T2\&, decltype(std::get<1>(std::forward<P>(p)))>} is \tcode{true}.
    \end{itemize}
...
\begin{itemdescr}
    ...
    \begin{codeblock}
        return uses_allocator_construction_args<T>(alloc, piecewise_construct,
        forward_as_tuple(@\changed{pr.first}{std::get<0>(std::forward<Pair>(pr)}@),
        forward_as_tuple(@\changed{pr.second}{std::get<1>(std::forward<Pair>(pr)}@);
    \end{codeblock}
\end{itemdescr}
