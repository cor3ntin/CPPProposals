...
The changes proposed in this paper makes the use of std::pair unecessary in new code.
...
\item Add the wording modification to zip and all associative containers.
\item Remove the modification to associative containers
\item Conserve the existing tuple and pair constructors, assignment, and comparison operators. The proposed changes to tuple and pair are now additions. This is both because implementers cannot remove them because of ABI, and because the proposed constructors are not foud for classes that inherit publicly from pair or tuple. This removes some concern for breaking changes.
\end{itemize}
...
Associatives containers are already specified to be usable with elements convertible to their 
...
\begin{itemize}
    \item It is based on C++20, so the proposed changes to zip have not been made.
    \item Pair and tuple constructors were modified (instead of having new ones)
\end{itemize}

I'm working on completing the implementation.
...
However, associatives containers are already specified to be usable with elements convertible to their 
\tcode{value_type}, so all constructors and methods can be used with \tcode{tuple-like}, except those taking an
\tcode{initializer_list} as parameter.
...
= @\placeholder{tuple-like}@<T> && tuple_size_v<T> == 2;
...
\begin{itemdescr}
    ...
    \begin{itemize}
        \item \tcode{sizeof...(Types)} equals \tcode{tuple_size_v<UTuple>} and
        \item \tcode{is_assignable_v<$\tcode{T}_i$\&, element_t<$i$, UTuple>>} is \tcode{true} for all $i$..
    \end{itemize}
...
\begin{itemdescr}
    ...
    \begin{itemize}
        \item
        \tcode{sizeof...(Types)} equals \tcode{tuple_size_v<UTuple>} and
        \item \tcode{is_assignable_v<const $\tcode{T}_i$\&, element_t<$i$, UTuple>\&\&>} is \tcode{true} for all $i$.
    \end{itemize}
...
\begin{itemdescr}
    ...
    \tcode{\changed{sizeof...(UTypes)}{tuple_size_v<remove_cvref_t<decltype(u)>>}}.
