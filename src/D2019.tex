...
\subsection{R8}
\begin{itemize}
\item Constrain the type \tcode{name_hint} (to accept only \tcode{char}) instead of its constructor.
\item Specify that the encoding of \tcode{name_hint} is the literal encoding.
...
\tcode{char8_t} support was present based on the assumption that, given that
a thread name is textual data, and because it matches the capability of underlying platforms,
we should support Unicode. And there is no point in supporting \tcode{wchar_t/char16_t/char32_t} as
...
(We are keeping an interface taking \tcode{basic_string_view<T>} rather than \tcode{string_view} in order to facilitate such future evolution in a non abi-breaking manner.)
...
The set of thread attribute types contains \tcode{thread::name_hint}, \tcode{thread::stack_size_hint} and an implementation-defined set of additional thread-attributes.

When an object of a thread attribute type is passed to a \tcode{thread} or \tcode{jthread} constructor, it may
affect the new thread's behavior.
...
public:
    constexpr explicit
    name_hint(basic_string_view<T> n) noexcept;
...
to set the name of a thread for debugging purposes or platform-specific display mechanisms.

\recommended Implementations should not store the value of the \tcode{name_hint} attribute in the \tcode{thread} or \tcode{jthread} object.

\recommended \tcode{name} is interpreted as a string in the literal encoding.
...
% \tcode{n} represents a string in the literal encoding.

\effects Initializes \tcode{name} with \tcode{n}.
\end{itemdescr}
...
\tcode{stack_size_hint} can be used to configure a desired size in bytes for platform-specific storage required for a thread. \begin{note}
Such storage is typically used for variables with automatic storage duration.	
\end{note}

%as if by POSIX \mbox{\tcode{pthread_attr_setstacksize}}.\\
The stack size set by the implementation may be adjusted up or down to meet platform-specific requirements.

If \tcode{size} is zero the thread attribute is ignored.
...
\begin{itemdescr}
\effects Initializes \tcode{size} with \tcode{s}.
\end{itemdescr}
...
\begin{itemize}
\begin{addedblock}
\item \tcode{Args} is not an empty pack, and
\end{addedblock}
...
\begin{itemize}
    ...
    \item \tcode{(is_constructible_v<decay_t<\added{F}Args>, \added{F}Args> \&\& ...)}, \removed{and}
\begin{addedblock}
...
\begin{codeblock}
    invoke(auto@(std::forward<F>(f)), auto@(std::forward<@\added{F}@Args>(@\added{f}@args))...)
\end{codeblock}
with the [...] is ignored.
\begin{note}
...
\begin{addedblock}
The parameters \tcode{attrs...} can affect the behavior of the new thread ([thread.attributes]).
\end{addedblock}
...
\begin{itemdecl}
template<@\removed{class F,}@ class... Args>
explicit jthread(@\removed{F\&\& f,}@ Args&&... args);
\end{itemdecl}
\begin{itemdescr}
\constraints
\begin{itemize}
...
\item \tcode{(is_constructible_v<decay_t<\added{F}Args>, \added{F}Args> \&\& ...)}, \removed{and}
\begin{addedblock}
%    \item \tcode{(!is_lvalue_reference_v<Attrs> \&\& ...)}, and
\end{addedblock}
\item \tcode{is_invocable_v<decay_t<F>, decay_t<\added{F}Args>...> ||
is_invocable_v<decay_t<F>, stop_token, decay_t<\added{F}Args>...>} \changed{.}{, and}
\begin{addedblock}
    \item No type is present more than once in the pack \tcode{remove_cvref_t<Attrs>}. %\ednote{// is there a better way to say that ?}
\end{addedblock}
\end{itemize}
...
\begin{addedblock}
The parameters \tcode{attrs...} can affect the behavior of the new thread.
\end{addedblock}
