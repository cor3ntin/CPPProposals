...
\begin{itemize}
\item Let $i$ be the smallest value such that \tcode{is_invocable_v<decay_t<Args...[$i$]>>} is \tcode{true}.
If no such $i$ exists, the programm is ill-formed.

\item Let \tcode{F} be \tcode{Args...[$i$]}.
\item Let \tcode{f} be \tcode{args...[$i$]}.
\item Let \tcode{Attrs} a pack of the types \tcode{Args...[0]},...,\tcode{Args...[$i-1$]} if $i$ is not equal to $0$ and an empty pack otherwise.
\item Let \tcode{attrs} a pack of the expressions \tcode{args...[0]},...,\tcode{args...[$i-1$]} if $i$ is not equal to $0$ and an empty pack otherwise.
\item Let \tcode{FArgs} a pack of the types \tcode{Args...[$i+1$]},...,\tcode{Args...[sizeof...(Args)-1]} if $i$ is not equal to \tcode{sizeof...(args)-1} and an empty pack otherwise.
\item Let \tcode{fargs} a pack of the expressions \tcode{args...[$i+1$]},...,\tcode{args...[sizeof...(args)-1]} if $i$ is not equal to \tcode{sizeof...(args)-1} and an empty pack otherwise..

% \ednote{Do we need to to be more explicit about }
...
\begin{itemize}
    ...
    \item \tcode{(is_constructible_v<decay_t<\added{F}Args>, \added{F}Args> \&\& ...)},
\begin{removedblock}
...
\begin{addedblock}
    \item No potentially cv-qualified type is present more than once in \tcode{Attrs}.
\end{addedblock}
\end{itemize}
...
\begin{addedblock}
\tcode{attrs} can be used to tailor the thread with additional implementation-defined behaviors. Thread attributes parameters unknown to the implementation are ignored. (see [thread.attributes]).
\end{addedblock}
