...
\begin{codeblock}
    ...
    template <typename T>
    name_hint(std::basic_string<T>) -> name_hint<T>;
...
Implementations should not store the value of the \tcode{name_hint} attribute in the \tcode{std::thread} or \tcode{std::jthread} object.
...
\begin{itemdescr}
\constraints
\tcode{std::same_as<T, char> || std::same_as<T, char8_t>} is \tcode{true}.
