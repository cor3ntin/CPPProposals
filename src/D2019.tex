...
We propose adding a new constructor to \tcode{std::thread} and \tcode{std::jthread}
of the form \tcode{thread(\placeholder{thread-attribute}..., invokable)} where attribute is
...
This interface is made possible by the realization that lambdas alleviate the needs for \tcode{(j)thread} to support parameter forwardings,
...
A \href{https://github.com/cor3ntin/llvm-project/tree/thread_name_p2019}{prototype implementation} for libc++ (supporting only POSIX) threads has been created to validate the design.
...
to set the name of a thread as if per a call to the POSIX function \tcode{pthread_setname_np}.
...
\recommended
Configure a stack size as if by POSIX \mbox{\tcode{pthread_attr_setstacksize()}}.\\
...
\begin{addedblock}
In the second form, \tcode{attrs} can be used to define implementation-defined behaviors. Thread attributes parameters unknown to the implementation are ignored. (see [thread.attributes]).
\end{addedblock}
