...
The index of a pack indexing expression or specifier is an integral constant expression between 0 and \tcode{sizeof...(pack)}.
Empty packs can't be indexed.

This paper does not add support for indexing from the end (as an alias of \tcode{T...[sizeof...(T)-N]}).
Indeed, a negative index (\tcode{T...[-1]}) would be surprising.
...
auto find_convertible_in_pack;
 ...
 // if find_convertible_in_pack<Foo, Types> is -1, T will be the last type, erronously.
...
The solution for indexing from the end is to provide a specific syntax, for example, C\# uses \tcode{\^{}} to mean "from the end", and Dlang interprets \tcode{\${}}
as the size of the array it appears in
\begin{colorblock}
using Foo = T...[0];
...
Given that there are different alternatives, all of which can be added later, and for which we do not have usage experience, this proposal does not support
from-the-end indexing.

\subsection{Indexing a pack of types}
...
We simply always consider packs indexing non-deduced contexts.

\subsection{Indexing a pack of expressions}

The intent is that a pack indexing expression behaves exactly as the underlying expression would.
In particular, \tcode{decltype(id-dexpression) and \tcode{decltype(pack-indexing-expression)}} behave the same.
...
\item Pack Slicing (Returning a subset of a pack as an unexpanded packs)
\item Packs of univeral template parameters could be indexed in the same way.
\end{itemize}


\section{Potential impact on existing code}

In C++11, \tcode{T... [N]} declares a pack of arrays of size \tcode{N};
...
This proposal is inspired from features implemented in the Circle compiler (with the same syntax).
The provided wording is based on an implementation in a fork of clang, which is available on compiler explorer.

The implementation is available on \href{https://compiler-explorer.com/z/WKobTEq6x}{Compiler Explorer}.


As of the writting of the paper, the implementation is known not to support pack indexing inside of the expansion of another template parameters.
We hope to fix these issues before Kona.
...
If a name, \grammarterm{template-id}, \added{pack-indexing-specifier,} or \grammarterm{decltype-specifier}
...
\begin{note}
    ...
    A \grammarterm{type-name} \added{, \grammarterm{pack-index-type}, } or \grammarterm{decltype-specifier}
...
\begin{bnf}
    ...
    \added{pack-indexing-specifier \terminal{::}}\br
...
\added{,
\grammarterm{nested-name-specifier} \opt{\keyword{template}} \tcode{\~}
\grammarterm{pack-indexing-specifier}
or \grammarterm{pack-indexing-specifier} \tcode{::} \tcode{\~} \grammarterm{type-name}
}
.
...
\begin{bnf}
    \nontermdef{pack-indexing-expression}\br
    id-expression \terminal{...} \terminal{[} constant-expression \terminal{]}  \br
\end{bnf}

The \grammarterm{id-expression} in a \grammarterm{pack-indexing-expression} shall denote a pack.
...
The index of a \grammarterm{pack-indexing-expression} is the value of its \grammarterm{constant-expression}.

A \grammarterm{pack-indexing-expression} denotes the ${index}$\textsuperscript{th} expression in a pack ([temp.variadic]).
A \grammarterm{pack-indexing-expression} is a pack expansion ([temp.variadic]).
...
\added{pack-indexing-specifier}\br
...
\begin{simpletypetable}
    ...
    \added{\grammarterm{pack-indexing-specifier}} & \added{the type as defined in [dcl.type.pack.indexing]} \\
...
\begin{bnf}
    \nontermdef{pack-indexing-specifier}\br
    typedef-name \terminal{...} \terminal{[} constant-expression \terminal{]}  \br
\end{bnf}

The \grammarterm{typedef-name} in a \grammarterm{pack-indexing-specifier} shall denote a pack.

The \grammarterm{constant-expression} shall be an integral constant expression.
The \grammarterm{constant-expression} shall evaluate to a value $V$ such that 0 <= $V$ < \tcode{sizeof...(typedef-name)}.

The index of a \grammarterm{pack-indexing-specifier} is the value of its \grammarterm{constant-expression}.

A \grammarterm{pack-indexing-specifier} denotes the type of the ${index}$\textsuperscript{th} \tcode{typedef-name} in a pack ([temp.variadic]).
A \grammarterm{pack-indexing-specifier} is a pack expansion ([temp.variadic]).
...
\begin{addedblock}
\begin{note}
A \grammarterm{pack-indexing-expression} is an \grammarterm{id-expression}.
\end{note}
\end{addedblock}
...
\begin{example}
    \begin{codeblock}
        ...
        \begin{codeblock}
        [](auto... pack){
            decltype(pack...[0]); // // type is \tcode{int}
            decltype((pack...[0]));  // type is \tcode{int\&}
        }(0);
        \end{codeblock}
        \end{addedblock}
...
\begin{itemize}
    ...
    \begin{addedblock}
    \item In a \grammarterm{pack-indexing-expression} ; the pattern is an
    \grammarterm{id-expression}.
    \item In a \grammarterm{pack-indexing-specifier} ; the pattern is an
    \grammarterm{identifier}.
    \end{addedblock}
...
When instantiating a \grammarterm{pack-indexing-expression} $P$,
let $K$ be the index of $P$.
The instantiation of $P$ is the \grammarterm{id-expression} \tcode{$\mathtt{E}_K$}.


When instantiating a \grammarterm{pack-indexing-specifier} $P$,
let $K$ be the index of $P$.
...
\tcode{T...[$n$]} is now a \grammarterm{pack-indexing-specifier}.
\rationale
