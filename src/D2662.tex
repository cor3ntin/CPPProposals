...
Over the years, different syntaxes have been considered by various authors:
...
\item Pack elements can be array-like or tuple-like,  so we need to be careful about ambiguities. In particular, the subscript syntax is not viable.
Indeed, indexing a pack of arrays and indexing each array in a pack are different operations (both of which are useful), and so using the subscript syntax would be ambiguous
(or lead to code that cannot be expressed) [\href{https://compiler-explorer.com/z/E86h8eMG1}{See this example}].
\end{itemize}
...
\paper{P2671R0} proposes a syntax (rather arbitrarily on the fact it's currently not used), to create a "pack object", or rather, to instruct the compiler to manipulate
...
Any syntax would be better than \href{https://twitter.com/incomputable/status/1590733659694583808}{the status quo}.
However, having done an analysis of different options, the original choice \tcode{Pack...[N]} still seems to be the better option, as it is consistent with existing pack features.
...
As of the writing of the paper, the implementation is known not to support pack indexing inside of the expansion of other template parameters.
