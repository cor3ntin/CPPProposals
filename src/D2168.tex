...
template <class Ref, std::common_reference_with<Ref> Value = std::remove_cvref_t<Y>, 
          class Allocator = void>
class generator  {
...
\begin{itemize}
\item \tcode{coroutine_traits<generator<Ref, Value, Alloc>::promise_type>>} is valid an denotes a type,
...
public:
    ...
    suspend_always yield_value(reference && value) n
        noexcept(is_nothrow_copy_constructible_v<reference, T>));
...
\begin{itemdecl}
suspend_always initial_suspend() noexcept const;
\end{itemdecl}
...
\begin{itemdecl}
suspend_always yield_value(reference && value) 
    noexcept(is_nothrow_copy_constructible_v<reference, T>));
\end{itemdecl}
\begin{itemdecl}
template <class T>
...
Variables with automatic storage duration in the scope of the coroutine represented by \tcode{g.coroutine_} are destroyed before variables with automatic storage duration in the scope of the coroutine denoted by this coroutine.
...
std::suspend_always yield_value(elements_of<R>&& rng) noexcept;
\end{itemdecl}
...
\begin{itemdescr}
\effects Allocates the coroutine frame with \tcode{std::allocator}.
\end{itemdescr}
...
\begin{itemdescr}
\effects Allocates the coroutine frame with \tcode{Allocator}.
\end{itemdescr}
...
\mandates
\begin{itemize}
\item \tcode{same_as<Allocator, void> || same_as<Allocator, Alloc>} is \tcode{true},
\item \tcode{Alloc} meets the \tcode{Cpp17Allocator} requirements,
\end{itemize}   
    
\effects Allocates the coroutine frame with \tcode{Alloc}.

\begin{note}
If \tcode{std::allocator_traits<Alloc>::is_always_equal::value} is \tcode{false} or if \tcode{is_default_constructible<Alloc> is \tcode{false}}, \tcode{alloc} is stored in the coroutine frame.
\end{note}
...
\begin{itemdescr}
Deallocate the coroutine frame with the allocator that was use to allocate it.
\end{itemdescr}
...
\begin{itemdescr}
\effects
Initializes \tcode{coroutine_} with \tcode{exchange(other.coroutine_, nullptr)}.
\end{itemdescr}
...
\begin{itemdescr}
\effects
Equivalent to \tcode{coroutine_ = exchange(other.coroutine_, nullptr);}
\end{itemdescr}
