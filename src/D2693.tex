...
than a \tcode{ostream} \tcode{<<} overload. There is no accessor of any kind, so the only well-defined way to extract a \tcode{thread::id}
is to use streams
...
\tcode{std::stacktrace} and \tcode{std::stacktrace_entry} would have both a formatter and a \tcode{to_string} methods,
...
\begin{itemdescr}
\effects Writes the same description that would be produced by \tcode{to_string(ste)} into \tcode{ctx.out()}.
...
\begin{itemdescr}
\effects Writes the same description that would be produced by \tcode{to_string(st)} into \tcode{ctx.out()}.
