...
We provide wording for both options.
\end{itemize}
...
\begin{addedblockTwo}
For \tcode{formatter<stacktrace_entry>}, \grammarterm{format-spec} is empty.

A \tcode{stacktrace_entry} object \tcode{se} is formatted as if by passing it to \tcode{to_string(se)} and copying the returned string through the output iterator of the context.
\end{addedblockTwo}

\ednote{Option 2 (support padding an alignement)}

\begin{addedblockThree}
...
A \tcode{stacktrace_entry} object is formatted as if by passing it to \tcode{to_string(se)} to the output iterator with \grammarterm{stacktrace-entry-format-spec} applied.
...
A \tcode{basic_stacktrace} object \tcode{s} is formatted as if by passing it to \tcode{to_string(s)} and copying the returned string through the output iterator of the context.
\end{itemdescr}
...
A \tcode{thread::id} object is formatted by writing its text representation for \tcode{charT} to the output with \grammarterm{thread-id-format-spec} applied.
\end{itemdescr}
