...
\begin{itemdescr}
\grammarterm{format-spec} for \tcode{formatter<stacktrace_entry>} must be empty.
...
\begin{itemdescr}
\grammarterm{format-spec} for \tcode{formatter<basic_stacktrace<Allocator>>} must be empty.
...
If the \grammarterm{align} option is omitted it is defaulted to \tcode{>}.

A \tcode{thread::id} object is formatted by writing its text representation to the output with \grammarterm{thread-id-format-spec} applied.
\end{itemdescr}
