...
\ednote {Modify [basic.scope.scope]/p5}

A declaration is \grammarterm{name-independent} if its name is \tcode{_}
and it declares a variable with automatic storage duration, a structured binding not inhabiting a namespace scope, the variable introduced by an \grammarterm{init-capture}, or a non-static data member.


% A name-independent declaration is considered to be marked \tcode{maybe_unused} % cross reference.
% add note: can't have attribute on init-capture

% replace the attribute by its effect
...
\begin{addedblock}
\begin{note}
An \grammarterm{id-expression} that names a unique name-independent declaration is usable until
...
\rSec2[basic.link]{Program and linkage}%
...
either is reachable from the other, the program is ill-formed \added{unless B is name-independent and the \grammarterm{using-declaration} precedes B}.
...
\ednote{Add a new macro in \tcode{[tab:cpp.predefined.ft]} : \tcode{__cpp_placeholder_variables} set to the date of adoption}.
