...
\begin{addedblock}
A declaration is a \grammarterm{redeclarable placeholder entity} if its name is \tcode{_},
...
a \grammarterm{redeclarable placeholder entity}
is unused.
\end{addedblock}
...
that potentially conflict and one precedes the other\iref{basic.lookup} \added{unless both declarations are \grammarterm{redeclarable placeholder entities}}.

\begin{addedblock}
\begin{note}
Because two \grammarterm{redeclarable placeholder entity} in the same scope
are \grammarterm{ambiguous}, an \grammarterm{id-expression} that denotes a
\grammarterm{redeclarable placeholder entity} is ill-formed.
\end{note}
\end{addedblock}
...
\begin{example}
	\begin{codeblock}
		void f() {
			...
			@\added{    int _;}@
			@\added{    int _;                  // ok}@
			@\added{    _ = 0;                 // error: id-expression '_' denoting a \grammarterm{redeclarable placeholder entity}}@
		}
