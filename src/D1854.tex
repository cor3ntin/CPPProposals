...
Narrow multicharacters literals such as \tcode{'intl'} are widely used. We are not proposing to removing them.
However, 'é' (e, ACUTE ACCENT) or '\emoji{rainbow}' (grapheme cluster), read as single characters.
...
\item It makes multicharacters literals slightly less confusing as it is dificile to imagine how multiple codepoints over 0x80 fcould be stuffed into an \tcode{int}.
in any sensible way.
\end{itemize}
...
GCC, Clang ICC(EDG) emit a warning for any multi-characters literals in general. They also emit a warning when the computed value exceed the size of \tcode{int}. 
...
\ednote{The \removedprev{Magenta text with blue squigly lines} correspond to wording previously removed by \paper{P2362R3}. This wording should stay removed.}
