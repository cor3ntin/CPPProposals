...
In phase 1 of lexing, and when describing constraints on \grammarterm{universal-character-name}, we use the term "Unicode scalar value"
...
\changed{Changes in green}{and red} are the addition and removal respectively associated to the replacement of the term "character".
...
\begin{enumerate}
    ...
    each pair of \changed{characters}{Unicode scalar values} in the input sequence consisting of
    ...
    \added{abstract} characters are mapped, in an
    \impldef{mapping physical source file characters to translation character set} manner,
    to a sequence of \changedTranslation{translation character set elements}{Unicode scalar values}\removed{\iref{lex.charset}},
    representing end-of-line indicators as new-line characters.

    \item
    \indextext{line splicing}%
    If the first \changedTranslation{translation character}{Unicode code point} is \unicode{feff}{byte order mark},
    it is deleted.
    Each sequence of \changed{a backslash character (\textbackslash)}{\unicode{005C}{backslash}}
    ...
    a \changed{character}{Unicode code point} sequence that matches the
    ...
    \begin{wfootnote}
        ...
        ending in the first portion of a multi-\changed{character}{Unicode code point} token that requires a
        terminating sequence of \changed{character}{Unicode code point}{s}, such as a \grammarterm{header-name}
        that is missing the closing \tcode{"}
        or \tcode{>}. A partial comment
        would arise from a source file ending with an unclosed \tcode{/*}
        comment.
    \end{wfootnote}
    Each comment is replaced by one \changed{space character}{\unicode{0020}{SPACE} character}. New-line characters are
    retained. Whether each nonempty sequence of whitespace characters other
    than new-line is retained or replaced by one \changed{space character}{\unicode{0020}{SPACE} character} is
    unspecified.
    As \changed{characters}{Unicode code points} from the source file are consumed
    ...
    \changed{characters}{code points} into preprocessing tokens is context-dependent.
    \begin{example}
...
\pnum
The \defnadj{basic}{character set} is a subset of the \changedTranslation{translation}{Unicode} character set,
consisting of 96 characters as specified in [lex.charset.basic].
\begin{note}
    Unicode short names are given only as a means to identifying the \changed{character}{code point};
...
other \changed{characters}{Unicode code points}.

\begin{bnf}
    \nontermdef{n-char} \textnormal{one of}\br
    \textnormal{\changedTranslation{any member of the translation character set}{Unicode code point} except the \unicode{007d}{right curly bracket} or new-line character}
\end{bnf}
...
designates the \changed{character}{Unicode code point} named by its \grammarterm{n-char-sequence}.
A \changed{character}{Unicode code point} is so named if the \grammarterm{n-char-sequence} is equal to
\begin{itemize}
    \item
...
\begin{note}
    A sequence of \changed{characters}{code points} resembling a \grammarterm{universal-character-name} in an
...
all \added{abstract} characters of the basic character set,
...
\changed{Characters}{Unicode code points} in a \grammarterm{character-literal}
...
\begin{note}
    A \changed{character}{Unicode code point} not in the basic literal character set
...
\begin{bnf}
    ...
    \textnormal{each non-whitespace \changed{character}{Unicode code point} that cannot be one of the above}
\end{bnf}
...
operators and punctuators, and single non-whitespace \changed{characters}{Unicode code points} that do not lexically
...
If any \changed{characters}{Unicode code point} not in the basic character set matches the last category,
...
given \changed{character}{Unicode code point}:
\begin{itemize}
    \item
    \indextext{literal!string!raw}%
    If the next \changed{character}{Unicode code point} begins a sequence of \changed{characters}{Unicode code points} that could be the prefix
    ...
    of \changed{characters}{Unicode code points} that matches the raw-string pattern
    \begin{ncbnf}
        \opt{encoding-prefix} \terminal{R} raw-string
    \end{ncbnf}

    \item Otherwise, if the next three characters are \tcode{<::} and the subsequent \changed{character}{Unicode code point}
    ...
    \changed{characters}{Unicode code points} that could constitute a preprocessing token, even if that
...
\begin{wfootnote}
    ...
    ``digraph'' (token consisting of two \changed{characters}{Unicode code points}) is not perfectly
    ...
    \tcode{\%:\%:} and of course several primary tokens contain two
     \changed{characters}{Unicode code points}. Nonetheless, those alternative tokens that aren't lexical
...
\begin{note}
    ...
    are treated just like other \changed{characters}{Unicode code points}. Similarly, the comment
...
\begin{bnf}
    \nontermdef{h-char}\br
    \textnormal{any \changedTranslation{member of the translation character set}{Unicode code point} except new-line and \unicode{003e}{greater-than sign}}
\end{bnf}
...
\begin{bnf}
    \nontermdef{q-char}\br
    \textnormal{any \changedTranslation{member of the translation character set}{Unicode code point} except new-line and \unicode{0022}{quotation mark}}
\end{bnf}
...
\begin{wfootnote}
    Thus, a sequence of \changed{characters}{Unicode code points}
    that resembles an escape sequence can result in an error, be interpreted as the
    \changed{character}{Unicode code point} corresponding to the escape sequence, or have a completely different meaning,
...
\begin{bnf}
    ...
    nondigit\br
    \textnormal{\changed{an element of the translation character set of class}{a Unicode code point with the Unicode property} XID_Start}
\end{bnf}
...
\begin{bnf}
    ...
    digit\br
    nondigit\br
    \textnormal{\changed{an element of the translation character set of class}{a Unicode code point with the Unicode property} XID_Continue}
\end{bnf}
...
\begin{wfootnote}
    ...
    \tcode{\textbackslash u} in a \grammarterm{universal-character-name}. \removed{Extended
...
%\ednote{The footnote needs more massaging. what are we  trying to say by "significant character", exactly?}
...
\begin{itemize}
    ...
    an uppercase letter \added{in the basic character set}
    \indextext{uppercase}%
    is reserved to the implementation for any use.

    \ednote{"letter" here is very confusing, but existing implementations and the history of the wording strongly imply the intent}
...
\begin{bnf}
    ...
    \textnormal{any \changedTranslation{member of the translation character set}{Unicode code point} except the \unicode{0027}{apostrophe},}\br
...
that specifies a \changed{character}{Unicode code point}
...
\begin{note}
    ...
    the specified \changed{character}{Unicode code point} lacks representation in
...
\begin{itemize}
    ...
    \grammarterm{universal-character-name}
    is the code unit value of the specified \changed{character}{Unicode code point}
    as encoded in the literal's associated character encoding.
    \begin{note}
        If the specified \changed{character}{Unicode code point} lacks
...
The \changed{character}{Unicode code point} specified by a \grammarterm{simple-escape-sequence}
...
\begin{floattable}{Simple escape sequences}{lex.ccon.esc}
    {lll}
    \topline
    \lhdrx{2}{\changed{character}{Unicode code point}} &  \rhdr{\grammarterm{simple-escape-sequence}} \\ \capsep
...
\begin{bnf}
    ...
    \textnormal{any \changedTranslation{member of the translation character set}{Unicode code point} except the \unicode{0022}{quotation mark},}\br
    \bnfindent\textnormal{\unicode{005c}{reverse solidus}, or new-line character}
\end{bnf}
...
\begin{bnf}
    \nontermdef{r-char}\br
    \textnormal{any\changedTranslation{member of the translation character set}{Unicode code point}, except a \unicode{0029}{right parenthesis} followed by}\br
...
\changed{characters}{Unicode code points} as the initial \grammarterm{d-char-sequence}. A \grammarterm{d-char-sequence}
shall consist of at most 16 \changed{characters}{Unicode code points}.
...
\begin{itemize}
    \item
    The sequence of \changed{characters}{Unicode code points} denoted by each contiguous sequence of
    ...
    If a \changed{character}{Unicode code point} lacks representation in the associated character encoding,
    ...
    \begin{note}
        No \changed{character}{Unicode code point} lacks representation in any of the UCS encoding forms.
    \end{note}
    ...
    \begin{note}
        ...
        encoding each \changed{character}{Unicode code point} independently.
    \end{note}
    \item
...
\changed{characters}{Unicode code points} that could match that non-terminal.
...
\end{codeblock}
where \placeholder{n} is the source \changed{character}{code point} sequence $c_1c_2...c_k$.
\begin{note}
    The sequence
    $c_1c_2...c_k$ can only contain \changed{characters}{code points} from the basic character set.
\end{note}
...
\begin{codeblock}
    operator "" @\placeholder{X}@<'@$c_1$@', '@$c_2$@', ... '@$c_k$@'>()
\end{codeblock}
where \placeholder{f} is the source \changed{character}{code point} sequence $c_1c_2...c_k$.
\begin{note}
    The sequence
    $c_1c_2...c_k$ can only contain \changed{characters}{code points} from the basic character set.
\end{note}
...
\begin{itemize}
    \item they are \grammarterm{identifier}{s} composed of the same \changed{character}{Unicode code point} sequence, or
...
begins with the first \changed{character}{Unicode code point} in the source file
...
The first \changed{character}{Unicode code point} shall not be a \grammarterm{digit}.
The implementation may ignore distinctions of \removed{alphabetical} case.

\ednote{"case" is mentioned by Unicode. Lets not consider how an implementation can ignore casing in this paper :).}
...
backslash. The resulting sequence of \changed{characters}{Unicode code points} is processed through translation phase 3
...
\rSec1[implimits]{Annex B (normative) Implementation quantities}
...
\begin{itemize}
    ...
    \item%
    Number of
    \changed{characters}{Unicode code points} in an internal identifier\iref{lex.name}
    or macro name\iref{cpp.replace} [1\,024].
    \item%
    Number of
    \changed{characters}{Unicode code points} in an external identifier\iref{lex.name,basic.link} [1\,024].
    \item%
...
\changed{the translation character set}{Unicode}.
