...
Let \tcode{C} designate \grammarterm{concept-name}, and let \placeholder{Parameter} be the first \grammarterm{template-parameter} in the \grammarterm{template-head} of \tcode{C}, without the ellipsis (if any).
A \grammarterm{partially-applied-concept-argument} names an unnamed concept \tcode{X} whose \grammarterm{concept-definition} is

\begin{codeblock}
template <@\placeholder{Parameter}@ Injected>
concept X = @\tcode{C}@<Injected, @\grammarterm{template-argument-list}@>;
\end{codeblock}

Two \grammarterm{partially-applied-concept-argument}{s} that are partial-concept-argument-equivalent\iref{temp.type} name the same concept.

If the \grammarterm{template-head} of \tcode{C} declares a single non-pack template parameter or if the \grammarterm{constraint-expression} of \tcode{X} is not a valid \grammarterm{concept-id}, the program is ill-formed.
...
void f(all_of<concept A<42>, concept C<int>, concept D<int>> auto); // OK
void f(all_of<concept E<int>> auto); // error: E declares a single non-pack template parameter
void f(all_of<concept C<int, int>> auto); // error: the constraint-expression of the invented concept would be C<Injected, int, int>, which is not a valid concept-id.

template <typename T, template <typename, typename> concept... Cs>
concept all_bound = all_of<T, concept Cs<int>...>; // OK
\end{colorblock}
\end{example}
...
\ednote{Add a paragraph at the end of [temp.type].}

\begin{addedblock}
Two template \grammarterm{template-argument}{s} are \defn{partial-concept-argument-equivalent} if
\begin{itemize}
\item they are both \grammarterm{partially-applied-concept-argument}{s},
\item they have equivalent \grammarterm{template-argument-list}{s}, and
\item their \grammarterm{concept-name}{s} name the same concept.
\end{itemize}
\end{addedblock}
