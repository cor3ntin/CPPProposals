...
template<ranges::input_range Range>
using @\placeholder{range-mapped-type}@ = 
    remove_const_t<typename ranges::range_value_t<Range>::second_type>; // \expos
...
\begin{codeblock}
    ...
    concept @\placeholder{container-insertable}@ =  // \expos
...
\begin{itemize}
    ...
    \item \tcode{to<range_value_t<C>>(declval<range_reference_t<R>>())} is a valid expression whose
      type and value category models \tcode{convertible_to<range_value_t<C>>},
\end{itemize}
...
\effects
\begin{itemize}
\item Expression-equivalent to \tcode{ranges::to<\placeholder{DEDUCE_TYPE}(R)>(std::forward<R>(r), std::forward<Args>(args)...)}.
\end{itemize}
