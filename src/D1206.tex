...
Wording changes
\begin{itemize}
    \item Rename the variable \tcode{range} to \tcode{rg}
    \item Fix a number of typos and presentation issues
    \item Fix the table of requirements to require \placeholder{Cpp17EmplaceConstructible} where appropriate, use  \tcode{*ranges::begin(rg)} instead of \tcode{range_reference_t<T>} as we are referring to a value rather than a type
    \item Clarify \tcode{vector} constructor complexity
    \item \tcode{ranges::from_iterator_t} was incorrectly spelled in a number of places
    \item Fix incorrect \tcode{ranges::} and \tcode{std::} qualifications
    \item Fix added deduction guides to take ranges by forwarding ref (which is needed to protect against array decay)
    \item Rename \placeholdernc{range-compatible-with-container-range-construction} to \placeholdernc{container-compatible-range}
    \item Use \placeholdernc{container-compatible-range} in the description of requirement tables.
    \item Constrain \tcode{ranges::to} and container \tcode{from_range_t} overload with \tcode{convertible_to} consistently. The concern was that
    if \tcode{ranges::to} is not properly constrained, it might try to construct improperly constrained non-standard types.
    \item Remove unnecessary constraints from \tcode{basic_string::assign_range}
    \item Add missing \tcode{constexpr} in a few places in \tcode{vector}
    \item Add missing \tcode{std::forward} in a few places
    \item Remove unecessary constraints on basic_string::append_range/replace_with_range
    \item simplify the reservable-container concept specification
    \item make ranges::to concepts into constexpr variables to make it clearer no semantic constraints is expected
    \item simplify and fix \tcode{ranges::to} wording and description
    \item introduce \tcode{__cpp_lib_ranges_to_container} instead of bumping  \tcode{__cpp_lib_ranges}
\end{itemize}
...
\begin{libreqtab4b}
    ...
    \begin{addedblock}
        ...
        \effects Constructs an empty container and inserts each element from \tcode{rg} into it.
...
\pnum
\effects
Inserts a copy of each element of \tcode{rg} at the beginning of the list. \begin{note}
...
\tcode{push_front}, \added{\tcode{prepend\char`_range}}, \tcode{pop_front}, and \tcode{emplace_front} member functions, which are not
...
using @\placeholder{range-key-type}@ = 
    remove_const_t<typename ranges::range_value_t<Range>::first_type>; // \expos

template<ranges::input_range Range>
using @\placeholder{range-mapped-type}@ = typename ranges::range_value_t<Range>::second_type; // \expos

template<ranges::input_range Range>
using @\placeholder{range-to-alloc-type}@  =  
...
template<ranges::input_range R,
    class Compare = less<@\placeholder{range-key-type}@<R>,
    class Allocator = allocator<@\placeholder{range-to-alloc-type}@<R>>>
map(from_range_t, R&&, Compare = Compare(), Allocator = Allocator())
    -> map<@\placeholder{range-key-type}@<R>, @\placeholder{range-mapped-type}@<R>, Compare, Allocator>;
\end{codeblock}
\end{addedblock}
...
-> map<@\placeholder{range-key-type}@<R>, @\placeholder{range-mapped-type}@<R>, less<@\placeholder{range-key-type}@<R>>, Allocator>;
\end{codeblock}
\end{addedblock}
\begin{codeblock}
...
-> multimap<@\placeholder{range-key-type}@<R>, @\placeholder{range-mapped-type}@<R>, Compare, Allocator>;
\end{codeblock}
\end{addedblock}
...
-> multimap<@\placeholder{range-key-type}@<R>, @\placeholder{range-mapped-type}@<R>, less<@\placeholder{range-key-type}@<R>>, Allocator>;
\end{codeblock}
\end{addedblock}
\begin{codeblock}
...
\begin{codeblock}
template<ranges::input_range R,
    class Hash = hash<@\placeholder{range-key-type}@<R>>,
    class Pred = equal_to<@\placeholder{range-key-type}@<R>>,
    class Allocator = allocator<@\placeholder{range-to-alloc-type}@<R>>>
unordered_map(from_range_t, R&&, typename @\seebelow@::size_type = @\seebelow@,
    Hash = Hash(), Pred = Pred(), Allocator = Allocator())
-> unordered_map<@\placeholder{range-key-type}@<R>, @\placeholder{range-mapped-type}@<R>,
    Hash, Pred, Allocator>;
\end{codeblock}
\end{addedblock}
\begin{codeblock}
...
-> unordered_map<@\placeholder{range-key-type}@<R>,
        @\placeholder{range-mapped-type}@<R>, hash<@\placeholder{range-key-type}@<R>>,
        equal_to<@\placeholder{range-key-type}@<R>>, Allocator>;


template<ranges::input_range R, class Allocator>
unordered_map(from_range_t, R&&, Allocator)
-> unordered_map<@\placeholder{range-key-type}@<R>,
    @\placeholder{range-mapped-type}@<R>, hash<@\placeholder{range-key-type}@<R>>, equal_to<@\placeholder{range-key-type}@<R>>, Allocator>;

template<ranges::input_range R, class Hash, class Allocator>
unordered_map(from_range_t, R&&, typename @\seebelow@::size_type, Hash, Allocator)
-> unordered_map<@\placeholder{range-key-type}@<R>, @\placeholder{range-mapped-type}@<R>, Hash, equal_to<@\placeholder{range-key-type}@<R>>, Allocator>;
\end{codeblock}
\end{addedblock}
\begin{codeblock}
...
\begin{itemdescr}
    ...
    inserts elements from the range \range{f}{l} \changed{for the first form, or from the range \range{il.begin()}{il.end()} for the second form}{, \tcode{rg}, or \tcode{il}, respectively}.
...
Hash = Hash(), Pred = Pred(), Allocator = Allocator())
-> unordered_multimap<@\placeholder{range-key-type}@<R>,
    @\placeholder{range-mapped-type}@<R>,
    Hash, Pred, Allocator>;
\end{codeblock}
\end{addedblock}
\begin{codeblock}
...
-> unordered_multimap<@\placeholder{range-key-type}@<R>,
    @\placeholder{range-mapped-type}@<R>,
    hash<@\placeholder{range-key-type}@<R>>,
    equal_to<@\placeholder{range-key-type}@<R>>, Allocator>;
...
namespace std {
    ...
    @\added{template<ranges::input_range R, class Allocator>}@
    @\added{queue(from_range_t, R\&\&, Allocator)}@
    @\added{-> queue<ranges::range_value_t<R>, deque<ranges::range_value_t<R>, Allocator>>;}@
...
\end{itemdecl}
\begin{itemdescr}
    \effects Insert all elements of \tcode{rg} in \tcode{c}.
    
    \postconditions \tcode{is_heap(c.begin(), c.end(), comp)} is \tcode{true}.
\end{itemdescr}
\end{addedblock}
...
namespace std {
    ...
    @\added{-> stack<ranges::range_value_t<R>, deque<ranges::range_value_t<R>, Allocator>>;}@
...
\begin{itemdescr}
    ...
    as indicated in [container.seq.req].
\end{itemdescr}
...
namespace std {
    ...
    @\added{struct from_range_t \{ explicit from_range_t() = default; \};}@
...
The range conversion functions construct an object (usually a container) from
a range, by using a constructor taking a range, a \tcode{from_range_t} tagged constructor,
or a constructor taking a pair of iterators, or by inserting each element of the
range into the default-constructed object.

\tcode{ranges::to} is applied recursively, allowing the conversion of a range of ranges.

\begin{example}
\begin{colorblock}
    string_view str = "the quick brown fox";
    auto words = views::split(str, ' ') | to<vector<string>>(); 
    // returns vector<string>{"the", "quick", "brown", "fox"}
\end{colorblock}\end{example}

Let \placeholder{reservable-container} be defined as follows:

\begin{codeblock}
    template<class Container>
    constexpr bool @\placeholder{reservable-container}@ = // \expos
...
Let \placeholder{container-insertable} be defined as follows:
\begin{codeblock}
    template<class Container, class Ref>
    constexpr bool @\placeholder{container-insertable}@ =  // \expos
...
Let \placeholder{container-inserter} be defined as follows:

    \begin{codeblock}
    template <typename Ref>
    auto @\placeholder{container-inserter}@(C & c){  // \expos
...
\rSec2[range.utility.conversions.to]{\tcode{ranges::to}}
...
\returns An object of type \tcode{C} constructed from the elements of \tcode{r} in the following manner:

\begin{itemize}
\item If \tcode{convertible_to<range_reference_t<R>, range_value_t<C>>} is \tcode{true}: 

\begin{itemize}
\item If  \tcode{constructible_from<C, R, Args...>} is \tcode{true}:

\quad\quad  \tcode{C(std::forward<R>(r), std::forward<Args>(args)...)}

\item Otherwise if \tcode{constructible_from<C, from_range_t, R, Args...>} is \tcode{true}:

\quad\quad  \tcode{C(from_range, std::forward<R>(r), std::forward<Args>(args)...)}

\item Otherwise if
\begin{itemize}
    \item \tcode{common_range<R>} is \tcode{true},
    \item \tcode{\placeholder{cpp17-input-iterator}<iterator_t<R>>} is \tcode{true}, and
    \item \tcode{constructible_from<C, iterator_t<R>, sentinel_t<R>, Args...>} is \tcode{true}:
\end{itemize}

\quad\quad\quad  \tcode{C c(ranges::begin(r), ranges::end(r), std::forward<Args...>(args)...);}

\item Otherwise if
\begin{itemize}
\item \tcode{constructible_from<C, Args...>} is \tcode{true}, and
\item \tcode{\placeholder{container-insertable}<C, range_reference_t<R>>} is \tcode{true}:
\end{itemize}

\begin{codeblock}
        C c(std::forward<Args...>(args)...);
        if constexpr (sized_range<R> && @\placeholder{reservable-container}@<C>)
            c.reserve(ranges::size(r));
        ranges::copy(r, @\placeholder{container-inserter}@<range_reference_t<R>>(c));
\end{codeblock}

\end{itemize}

\item Otherwise if \tcode{input_range<range_reference_t<R>>} is \tcode{true}:
...
Let \placeholder{\tcode{input-iterator}} be an exposition-only type
\begin{codeblock}
struct @\tcode{\placeholder{input-iterator}}@ { //\expos
    ...
    @\tcode{\placeholder{input-iterator}}@& operator++();
    @\tcode{\placeholder{input-iterator}}@ operator++(int);
    bool operator==(const @\tcode{\placeholder{input-iterator}}@&) const;
};
\end{codeblock}

\begin{note}
\placeholder{\tcode{input-iterator}} meets the syntactic requirements of \tcode{Cpp17InputIterator}.
\end{note} 

Let \placeholder{DEDUCE_EXPR} be defined as follows:
\begin{itemize}
\item \tcode{C(declval<R>(), declval<Args>()...)} if that is a valid expression,
\item otherwise, \tcode{C(from_range, declval<R>(), declval<Args>()...)} if that is a valid expression,
\item otherwise, \tcode{C(declval<\placeholder{\tcode{input-iterator}}>(), declval<\placeholder{\tcode{input-iterator}}>(), declval<Args>()...)} if that is a valid expression,
\item otherwise, the program is ill-formed.
\end{itemize}
...
\returns A range adaptor closure object ([range.adaptor.object]) \tcode{f} that is a perfect forwarding call wrapper ([func.require]) with the following properties:
\begin{itemize}
\item It has no target object.
\item Its bound argument entities bound_args consist of objects of types \tcode{decay_t<Args>...} direct-non-list-initialized with \tcode{std::forward<Args>(args)...}, respectively.
\item Its call pattern is \tcode{to<C>(r, bound_args...)}, where \tcode{r} is the argument used in a function call expression of \tcode{f}.
\end{itemize}
\pnum
...
Add a new macro in \tcode{<version>} \tcode{__cpp_lib_ranges_to_container} set to the date of adoption.
The macro \tcode{__cpp_lib_ranges_to_container} is also present in <ranges>.
