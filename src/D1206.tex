...
\pnum
\complexity
Linear in \tcode{ranges::distance(rr)}.
\end{itemdescr}
...
\begin{itemdescr}
    ...
    otherwise:
    \begin{codeblock}
        ranges::copy(std::forward<R>(rg), std::back_inserter(c));
    \end{codeblock}
\end{itemdescr}
\end{addedblock}
...
\begin{codeblock}
    ...
    concept @\placeholder{container-insertabler}@ =  // \expos
        requires(Container& c, Ref&& ref) {
            requires requires{ c.push_back(std::forward<Ref>(ref)); } || requires{ c.insert(std::end(c), std::forward<Ref>(ref)); };
    };
};
\end{codeblock}
