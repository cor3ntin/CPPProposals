...
This fixes \paper{CWG2002}. The list is sufficient to address the needs of \paper{P2295R1}.
...
We intend that this paper addresses \paper{1655}.

\subsection{Line splicing}

This paper also addresses \paper{CWG1698}.
...
\begin{enumerate}
    ...
    backslash on any physical source line \added{but the last} shall be eligible for being part
    of such a splice.
    ...
    \removed{Each comment is replaced by one space character}. \clb{New-line characters} are
    retained. \removed{Whether each nonempty sequence of whitespace characters other
...
separated by \cwhitespace{whitespace;}\added{.}
\indextext{comment}%
...
form-feed), or both}. As described in [cpp], in certain
circumstances during translation phase 4, \cwhitespace{whitespace} (or the absence
thereof) serves as more than preprocessing token separation. \cwhitespace{Whitespace}
...
operators, and \changed{other separators}{whitespaces}.
...
\begin{bnf}
    ...
    \textnormal{// any character except} line-break\br
    \textnormal{/* any character */}\br
\end{bnf}
...
\end{removedblock}
\pnum
\changed{The characters \tcode{/*} start a comment, which terminates with the
characters \tcode{*/}. These c}{C}omments do not nest.
...
\begin{bnf}
    \nontermdef{h-char}\br
    \textnormal{any member of the source character set except \clb{new-line} and \terminal{>}}
\end{bnf}
...
\begin{bnf}
    ...
    \textnormal{any member of the basic source character set except the single-quote \terminal{'}, backslash \terminal{\textbackslash}, or \clb{new-line character}}
\end{bnf}
...
\begin{bnf}
    ...
    \textnormal{any member of the translation character set except the double-quote \terminal{"}, backslash \terminal{\textbackslash}, or \clb{new-line character}}
\end{bnf}
...
\changed{\textnormal{space}}{horizontal-whitespace-character, line-break}, \textnormal{the left parenthesis \terminal{(}, the right parenthesis \terminal{)}, the backslash \terminal{\textbackslash}\removed{, and the control characters\br
...
follows \cwhitespace{whitespace} containing at least one \clb{new-line character},
and is
...
is immediately followed by \cwhitespace{whitespace} containing a \clb{new-line character}.
\begin{note}
...
where all \cwhitespace{whitespace} separations are considered identical.
...
\pnum
\indextext{macro!replacement list}%
There shall be \cwhitespace{whitespace} between the identifier and the replacement list
...
\removed{Any} \cwhitespaces{whitespace characters} preceding or following the
...
\added{Any} \cwhitespace{Whitespace} before the first preprocessing token and after the last
...
the number of \clb{new-line characters} read or introduced
