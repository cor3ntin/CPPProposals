...
\begin{itemize}
    ...
    \item Rebase [cpp] on top of \paper{P2334R1}
    \item Rebase [lex.phases] on top of \paper{P2223R2}
    \item Rebase [lex.phases] on top of \paper{P2314R4}
\end{itemize}
...
With this, such diagnostic in pedantic mode is not necessary for conformance, but might still
...
\begin{enumerate}
    ...
    Each sequence of a backslash character (\textbackslash) immediately followed by zero or more
    \changed{whitespace characters other than new-line}{\grammarterm{horizontal-whitespace-character}{s}} followed by a \clb{new-line  character} is deleted, splicing
    ...
    undefined. A source file that is not empty and that does not end in a splice,
    ...
    unspecified.}  As characters from the source file are consumed to form the next preprocessing token (i.e., not being consumed as part of a \cwhitespace{comment or other forms of whitespace}), except when matching a \grammarterm{c-char-sequence}, \grammarterm{s-char-sequence}, \grammarterm{r-char-sequence}, \grammarterm{h-char-sequence}, or \grammarterm{q-char-sequence}, \grammarterm{universal-character-name}{s} are recognized and replaced by the designated element of the translation character set. The process of dividing a source file's
...
\begin{bnf}
    ...
    \textnormal{each \changed{non-whitespace character}{character that is not part of a \grammarterm{whitespace} and}  \changed{cannot be}{is not part of} one of the above}
\end{bnf}
...
\begin{bnf}
    ...
    \textnormal{any member of the translation character set except} line-break-character\br
\end{bnf}
...
\begin{bnf}
    ...
    \textnormal{any member of the translation character set except * immediately followed by /}\br
\end{bnf}

\begin{bnf}
    \nontermdef{line-break}\br
    line-break-character\br
    \textnormal{U+000D CARRIAGE RETURN immediately followed by U+000A LINE FEED}\br
\end{bnf}
...
A \grammarterm{whitespace} is the longest sequence of characters that could constitute a \grammarterm{whitespace}.
...
\begin{bnf}
    \nontermdef{h-char}\br
    \textnormal{any member of the source character set except \clbc{new-line} and \terminal{>}}
\end{bnf}
...
\begin{bnf}\obeyspaces
    ...
    \terminal{\# elif    } constant-expression \clb{new-line} \opt{group}\br
    \terminal{\# elifdef } constant-expression \clb{new-line} \opt{group}\br
    \terminal{\# elifndef} constant-expression \clb{new-line} \opt{group}
\end{bnf}
...
\begin{ncsimplebnf}\obeyspaces
    ...
    \terminal{\# ifndef  } identifier \clb{new-line} \opt{group}\br
    \indextext{\idxcode{\#ifndef}}%
    \terminal{\# elifdef } identifier \clb{new-line} \opt{group}\br
    \indextext{\idxcode{\#elifdef}}%
    \terminal{\# elifndef} identifier \clb{new-line} \opt{group}
