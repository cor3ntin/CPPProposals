...
This revision returns N when the range has no \tcode{reserve_hint}.
...
\begin{itemdescr}
    ...
    models \libconcept{approximately_sized_range} only if \tcode{ranges::reserve_hint(t)} is amortized \bigoh{1}, and does not modify \tcode{t}.
\end{itemdescr}
\end{addedblock}
...
constexpr auto reserve_hint() {
    if constexpr (requires @\libconcept{approximately_sized_range}@<V>) {
        auto n = ranges::reserve_hint(@\exposid{base_}@);
        return ranges::min(n, static_cast<decltype(n)>(@\exposid{count_}@));
    }
    else return @\exposid{count_}@;
}

constexpr auto reserve_hint() const {
    if constexpr (requires @\libconcept{approximately_sized_range}@<const V>) {
        auto n = ranges::reserve_hint(@\exposid{base_}@);
        return ranges::min(n, static_cast<decltype(n)>(@\exposid{count_}@));
    }
    else return @\exposid{count_}@;
...
constexpr auto reserve_hint() requires @\libconcept{approximately_sized_range}@<V> {
    const auto s = ranges::reserve_hint(@\exposid{base_}@);
    const auto c = static_cast<decltype(s)>(@\exposid{count_}@);
    return s < c ? 0 : s - c;
}
...
Equivalent to:
\begin{codeblock}
    using ST = decltype(ranges::reserve_hint(@\exposid{base_}@));
    using CT = common_type_t<ST, size_t>;
...
class adjacent_transform_view : public view_interface<adjacent_transform_view<V, F, N>> {
    ...
    @\added{constexpr auto reserve_hint() requires \libconcept{approximately_sized_range}<InnerView>}@
    @\added{\{ return \exposid{inner_}.reserve_hint(); \}}@

    @\added{constexpr auto reserve_hint() const requires \libconcept{approximately_sized_range}<const InnerView>}@
    @\added{\{ \exposid{inner_}.reserve_hint(); \}}@
};
}
\end{codeblock}
...
\begin{itemdescr}
    ...
    Equivalent to:
    \begin{codeblock}
        return @\exposid{to-unsigned-like}@(@\exposid{div-ceil}@(ranges::reserve_hint(@\exposid{base_}@), @\exposid{n_}@));
    \end{codeblock}
\end{itemdescr}
\end{addedblock}
...
\begin{itemdescr}
    ...
    Equivalent to:
    \begin{codeblock}
        auto sz = static_cast<range_difference_t<R>>(ranges::reserve_hint(r)) - @\exposid{n_}@ + 1;
        if (sz < 0) sz = 0;
        return @\exposid{to-unsigned-like}@(sz);
    \end{codeblock}
\end{itemdescr}
...
Equivalent to:
\begin{codeblock}
    return @\exposid{to-unsigned-like}@(@\exposid{div-ceil}@(ranges::reserve_hint(@\exposid{base_}@), @\exposid{stride_}@));
\end{codeblock}
\end{itemdescr}
\end{addedblock}
