...
\begin{addedblock}
\grammarterm{attribute-token} that are not \grammarterm{attribute-scoped-token} as well as all top level \grammarterm{attribute-namespace} whose name is \tcode{std} or \tcode{std}  followed by one or more digits are reserved for the standard.
\end{addedblock}
