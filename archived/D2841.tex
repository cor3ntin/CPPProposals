...
\item Core review in Hagenberg.

Made fold expressions containing both concepts template parameters
and non-concept template parameters ill-formed:
...
\begin{itemize}
    ...
    \item atomic constraints\iref{temp.constr.atomic}, \removed{and}
    \item fold expanded constraints\iref{temp.constr.fold}\changed{.}{, and}
\begin{addedblock}
...
\begin{addedblock}
\item
The normal form of an expression F of the form \tcode{( E \tcode{Op} ... )} is as follows:
...
\begin{itemize}
\item
If \tcode{E} names any unexpanded concept template parameter pack $P_k$ that has corresponding template arguments in the parameter mapping of any atomic constraint (including concept-dependent constraints) of \tcode{E'},
...
\added{A synthesized template has the same \grammarterm{template-head} as its corresponding template template
parameter.}
...
\begin{itemize}
    \item
    it is a concept-id and  \added{its \grammarterm{concept-name} is dependent or} any of its arguments are dependent \added{[temp.dep.temp]},
    \item
...
{\tcode{<}$X$\tcode{>}
represents template argument lists where at least one argument contains an
$X$, where $X$ is one of \tcode{T}, \tcode{i}, \tcode{TT}, or \tcode{VV};}
and
\tcode{<>}
...
\ednote{Add a new macro in [tab:cpp.predefined.ft] : __cpp_template_parameters set to the
date of adoption} .
