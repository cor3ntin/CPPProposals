...
namespace std::ranges {
	...
	class filter_view<V, Pred>::iterator {
		...
		requires @\placeholder{has-arrow}@<iterator_t<V>> @\added{\&\& copyable<iterator_t<V>>}@;
...
\item Otherwise, \tcode{iterator_category} denotes \changed{\tcode{input_iterator_tag}}{C}.
\end{itemize}
...
Equivalent to:
\begin{codeblock}
   current_ = ranges::find_if(@\added{std::move(}@++current_@\added{)}@,
...
namespace std::ranges {
	...
	struct join_view<V>::iterator {
		...
		requires @\placeholder{has-arrow}@<iterator_t<Base>> @\added{\&\& copyable<iterator_t<Base>>}@;
...
\begin{itemize}
	...
	\item Otherwise, \added{if \placeholder{OUTERC} and \placeholder{INNERC} each model
	\tcode{derived_from<input_iterator_tag>},} \tcode{iterator_category} denotes \tcode{input_iterator_tag}.
	\item
...
\begin{itemdescr}
	\pnum
	\effects
	Initializes \tcode{outer_} with \tcode{\added{std::move(}outer\added{)}} and
...
\tcode{\libconcept{derived_from}<forward_iterator_tag>}, and \tcode{input_iterator_tag} otherwise.
\end{removedblock}
...
\item
  ...
  \tcode{\libconcept{derived_from}<forward_iterator_tag>};
\item
  otherwise, \tcode{iterator_traits<iterator_t<Base>>::iterator_category}.
\end{itemize}
\end{addedblock}
...
\begin{itemdescr}
	\pnum
	\effects
	Initializes \tcode{current_} with \tcode{\added{std::move(}i.current_\added{)}}.
\end{itemdescr}
