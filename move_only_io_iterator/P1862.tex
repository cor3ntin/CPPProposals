...
Therefore \tcode{subrange}, \tcode{common_view} and \tcode{counted_view} require copyable iterators.
...
\addedthree{The provided wording is for this option appears in green.}
...
\addedtwo{The provided wording for this option appears in violet.}
...
namespace std::ranges {
	...
	requires (@\addedthree{copyable<I> \&\& (}@ K == subrange_kind::sized || !sized_sentinel_for<S, I>@\addedthree{)}@)
	class subrange;
...
\begin{codeblock}
	namespace std::ranges {
		...
		requires @\addedthree{copyable<I> \&\&}@ (K == subrange_kind::sized || !sized_sentinel_for<S, I>)
		class subrange : public view_interface<subrange<I, S, K>> {
			...
			constexpr I begin() const @\addedtwo{requires forward_iterator<I>}@;
			@\addedtwo{[[nodiscard]] constexpr I begin() requires !forward_iterator<I>}@;
			...
			@\addedtwo{\& requires forward_iterator<I>}@;
			@\addedtwo{[[nodiscard]] constexpr subrange next(iter_difference_t<I> n = 1) \&\&;}@
		...
		@\addedtwo{template<size_t N, class I, class S, subrange_kind K>}@
		@\addedtwo{requires (N < 2)}@
		@\addedtwo{constexpr auto get(subrange<I, S, K>\&\& r);}@
	}
...
\begin{itemdescr}
	...
	\pnum
	\effects
	Initializes \tcode{begin_} with \tcode{\changedtwo{i}{std::move(i)}} and \tcode{end_} with
	\tcode{s}.
\end{itemdescr}
...
\indexlibrarymember{begin}{subrange}%
\begin{itemdecl}
	constexpr I begin() requires !forward_iterator<I>@;
\end{itemdecl}
...
\item Otherwise, \added{if \tcode{C} models copyable, }\tcode{iterator_category} denotes \tcode{input_iterator_tag}.
...
\added{if \tcode{C} models \tcode{copyable} then} \tcode{iterator_category} denotes \tcode{C}.
...
namespace std::ranges {
	...
	class drop_view : public view_interface<drop_view<R>> {
		...
		requires (@\added{forward_range<R> \&\&}@ !(@\placeholder{simple-view}@<R> && random_access_range<R>));
		constexpr auto begin() const
		requires @\added{(!forward_range<R> ||}@ random_access_range<const R> @\added{)}@;
...
requires (@\added{forward_range<R> \&\&}@ !(@\placeholder{simple-view}@<R> && random_access_range<R>));
constexpr auto begin() const
requires @\added{!forward_range<R> ||}@ random_access_range<const R>;
\end{itemdecl}
...
namespace std::ranges {
	...
	class drop_while_view : public view_interface<drop_while_view<R, Pred>> {
		...
		@\added{constexpr auto begin() requires forward_range<R>;}@
		constexpr auto begin() @\added{const requires !forward_range<R>}@;
...
the first \changed{call}{overload} caches the result within the \tcode{drop_while_view}
for use on subsequent calls.
\begin{note}
	Without this,
...
\begin{itemize}
	...
	\item Otherwise, \added{if \placeholder{OUTERC} and  \placeholder{INNERC} model = \tcode{coyable},} \tcode{iterator_category} denotes \tcode{input_iterator_tag}.
	\item
	\added{Otherwise, \tcode{iterator_category} denotes \tcode{output_iterator_tag}.}
\end{itemize}
...
...
The \grammarterm{typedef-name} \tcode{iterator_category} denotes \tcode{forward_iterator_tag} if
\tcode{iterator_traits<iterator_t<Base>>::iterator_category} models
\tcode{derived_from<forward_iterator_tag>}.
\begin{removedblock}
, and \tcode{input_iterator_tag} otherwise.
\end{removedblock}

\begin{addedblock}
Otherwise, \tcode{iterator_category} denotes \tcode{input_iterator_tag} if \tcode{iterator_t<Base>} models \tcode{copyable}}.
Otherwise, \tcode{iterator_category} denotes \tcode{output_iterator_tag}.
\end{addedblock}
...
\begin{itemize}
\item If \tcode{T} models \libconcept{input_or_output_iterator}\addedthree{ and  \libconcept{copyable}} and
