...
public:
    ...
    @\added{requires ranges::SizedRange<R> \&\& Same<iter_value_t<iterator_t<R>>, charT>}@
...
@\added{basic_string_view(const R\& r)}@
@\added{requires ranges::SizedRange<R>}@
@\added{\qquad -> basic_string_view<iter_value_t<iterator_t<R>>>;}@
...
\begin{itemdescr}
    ...
    \begin{itemize}
        ...
        \item \tcode{R} does not derive from an instantiation of  \tcode{std::basic_string}
        \item \tcode{R} does not derive from an instantiation of  \tcode{std::basic_string_view}
        \end{itemize}
    %
\end{itemdescr}
\end{addedblock}
