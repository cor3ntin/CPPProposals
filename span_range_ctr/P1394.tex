...
@\added{requires ranges::SizedRange<R> \&\& (\placeholder{forwarding-range}<R> ||  std::is_const_v<ElementType>)}@ 
...
\begin{itemdescr}
	...
	\pnum
	\ensures
	\tcode{size() == count \&\& data() == \changed{ptr}{\tcode{std::to_address(first)}}}.
...
\begin{itemdescr}
	...
	\pnum
	\ensures
	\tcode{size() == last - first \&\& data() ==  \tcode{std::to_address(first)}}.
...
requires ranges::SizedRange<R> && @{(\placeholder{forwarding-range}<R> || std::is_const_v<ElementType>)}@ &&
