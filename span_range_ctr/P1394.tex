...
constexpr span( @\removed{pointer ptr}@ @\added{It begin}@, size_type count);
...
\end{itemize}
    ...
    \begin{addedblock}
    \begin{itemize}
    	\item \tcode{size() == count} is \tcode{true}.
    	\item \tcode{to_address(first) == data()} is \tcode{true}.
...
\begin{itemdescr}
    ...
    \begin{itemize}
    	\item \tcode{size() == last - first} is \tcode{true}, and
    	\item \tcode{to_address(first) == data()} is \tcode{true}.
...
\begin{itemdescr}
    ...
    \begin{itemize}
       ...
       \item either \tcode{R} satisfies \tcode{\placeholder{forwarding-range}} or \tcode{is_const_v<element_type>} is true,
       ...
       \item \tcode{is_convertible_v<remove_reference_t<ranges::range_reference_t<R>>(*)[], element_type(*)[]>} is true
       \begin{note}The intent is to allow only qualification conversions of the iterator reference type to \tcode{element_type} \end{note}.
    \end{itemize}
