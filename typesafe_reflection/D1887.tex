...
To that end, user-defined-attributes are only parsed as such upon a call to \tcode{has_attribute} 
or \tcode{meta::attribute}.
...
For any attribute \tcode{A}, a call to \tcode{has_attribute<A>} or \tcode{meta::attribute<A>} on the reflection of an object \tcode{obj} is ill-formed if:
\begin{itemize}
    \item A is not defined
    \item \tcode{obj} has an attribute named "A" but that attribute cannot be evaluated as a valid C++ constant expression constructing an object of type \tcode{A}.
    \item \tcode{A} does not model \tcode{regular} or does not have a \tcode{constexpr} copy constructor and equality comparison operator.
\end{itemize}

Furthermore, a call to \tcode{meta::attribute<A>} on the reflection of an object \tcode{obj} is ill-formed if no attribute of that type appertains to \tcode{obj}.


\subsection{Non ignorable attributes}

The syntax \tcode{[[::foo::bar()]]} un-ambiguously designates a user-defined attribute and in this case, we could enforce that the attribute exists and the constructor expression at the point where the attribute is used.
