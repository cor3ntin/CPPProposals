...
namespace std::ranges {
	...
	&& Constructible<Cont, range_common_iterator_t<Rng>,
	   range_common_iterator_t<Rng>, Args...>;
	
	template <typename C, InputRange R, typename... Args>
	requires @\EXPO{convertible-to-container<Rng, C>}@ 
...
The rvalue overloads of the function ranges::to return an implementation defined object that can be 
passed to the pipe operator, and provide an implementation defined way of creating a container of 
the appropriate type. 
The container is constructed from the begin(range)/end(range)iterators pair.
\begin{note}
Implementations are encouraged to reserve the container's memory when the range is a \tcode{SizedRange}
\end{note}
...
\end{itemdecl}
\begin{itemdescr}
\returns \tcode{C\{\EXPO{common-iterator-type}(ranges::begin(r)), \\ \EXPO{common-iterator-type}(ranges::end(r)), std::forward<Args>(args)...\}} 
\end{itemdescr}
\begin{itemdecl}
template <template <typename...> typename C,  InputRange R, typename... Args> 
...
\returns \tcode{Ret\{\EXPO{common-iterator-type}(ranges::begin(r)), \\ \EXPO{common-iterator-type}(ranges::end(r)), std::forward<Args>(args)...\}} 
\end{itemize}
...
auto v5 = ints | to<vector>(ints);
auto v6 = ints | to<vector<int>>(ints);
