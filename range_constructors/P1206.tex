...
{\bf As explained in the "Design consideration", this proposal focuses on explicit construction and does not propose implicit container conversion.}
...
\begin{codeblock}
    ...
    std::ranges::copy(v, std::back_inserter(materialized));
\end{codeblock}
...
\subsection{Ranges and sentinels}
...
Whether \cc{std::vector} can be converted to and from \cc{std::string} in $\mathcal{O}(1)$ is an area of interest, notably for SG-16 - see \cite{P1072R1}.
...
We would like to thank the people who gave feedback on this paper, notably Casey Carter, Arthur O'Dwyer, Barry Revzin
and Tristan Brindle.\\
