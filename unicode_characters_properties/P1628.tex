...
static_assert(uni::cp_block(U'@\emoji{1F574}@') == uni::block::misc_pictographs);
static_assert(!uni::cp_is<uni::property::xid_start>('1'));
static_assert(uni::cp_is<uni::property::xid_continue>('1'));
static_assert(uni::cp_age(U'@\emoji{1F60D}@') == uni::version::v10_0);
...
\begin{itemize}
    \item Put the implementation in a single-header. Compile times can be optimized with the caveat of \tcode{cp_name} which in our implementation declares a few arrays for a total of 380KB - research is on-going to reduce this further but the minimum size should be upward of 200K.
